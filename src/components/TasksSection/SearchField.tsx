...
const SearchField: React.FC = () => {
  ...
  return (
    <div className="flex-1 col-span-3 row-start-2 md:pr-3">
      <form className=" relative md:max-w-xs w-full" autoComplete="off">
        ...
        <input
          ...
          className="inputStyles w-full bg-slate-100 dark:bg-[#2C2C2C]"
        />
        <Search className="absolute w-4 sm:w-5 right-4 top-3.5 text-slate-400" />
        {listResultsVisible && (
          <div className="absolute bg-slate-100 rounded-md w-full top-14 p-3 dark:bg-[#2C2C2C] z-10">
            {tasks.length ? (
              <>
                <ul>
                  {tasks.map((task) => (
