...
const InputCheckbox: React.FC<{
  ...
  return (
    <label className="mb-0 flex items-center cursor-pointer">
      <div className="mr-2 bg-slate-300/[.5] dark:bg-[#2c2c2c] w-5 h-5 rounded-full grid place-items-center border border-slate-300 dark:border-slate-700">
        {isChecked && (
          <span className="bg-[#357937] w-2 h-2 block rounded-full"></span>
        )}
      </div>
...
const ModalCreateTask: React.FC<{
  ...
  return (
    <Modal onClose={onClose} title={nameForm}>
      <form
        className="flex flex-col stylesInputsField overflow-y-auto"
      ...
      >
        ...
        <label>
          ...
          <select
            className="block w-full"
            value={priority}
            onChange={({ target }) => setPriority(target.value as "High" | "Medium" | "Low")}
          >
